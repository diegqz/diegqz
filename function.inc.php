...
function user_reg($name,$pass,$pass2){
	...
	$result = mysql_query("SELECT * FROM {$db}.{$table}users WHERE name='$name'");
	if(!$result){echo '选择数据表失败';}
	...
	if( !ereg('^[0-9a-zA-Z\_]*$',$name ) ){
			...
			else{echo'error!'.mysql_error().'错误产生的原因为：没有清空旧版数据，请删除两个数据表，重新安装iZzx_QQOL即可解决此问题！';}
			}
	}
//打开网址函数（无心问世出品）
function openu($url)
...
function add_onlinetime(){
    ...
    $time5= $time.' '.$deltime;
    return $time5;}
function add_online($sid,$onlinetime){
	global $host,$user,$password,$db,$table;
	$con=mysql_connect("$host","$user","$password")or die("无法连接服务器");
    mysql_select_db($db,$con);
	mysql_query("INSERT INTO {$table}online (id, url,time)
VALUES (NULL,'$sid','$onlinetime')");}
//删除在线QQ函数
